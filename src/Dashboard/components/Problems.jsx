...
const Loader = () => (
  <div className="w-full text-center ds-loading flex justify-center items-center">
    <div className="spinner-border text-primary" role="status">
      <svg aria-hidden="true" className="w-8 h-8 text-slate-200 animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <>
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <div>
          <h1 className="ds-title">Help Center Problems</h1>
          <p className="ds-subtitle">Build a better self-serve knowledge base for recurring issues.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {!loading && currentProblems.length === 0 ? (
          <div className="ds-empty col-span-full">
            <p className="text-lg font-semibold text-slate-700">No problems found</p>
            <p className="text-sm mt-2">Add a new problem to improve the help center experience.</p>
          </div>
        ) : (
          currentProblems.map((problem) => (
            ...
            <div
              key={problem.id}
              className="ds-card h-fit"
            >
              <section className='flex items-start justify-between p-4 '>
                <h2 className="w-[80%] text-lg font-semibold text-slate-800 mb-2 flex flex-col break-words">{problem.subject}
                {
                  problem.isSolved ? <span className='text-sm text-emerald-600'>Solved</span> : <span className='text-sm text-amber-600'>Pending</span>
                
                }
              <p className="text-sm text-slate-500 mb-4">{problem.type}</p>
              <p className="text-sm text-slate-600 mb-4">{problem.description }</p>
                </h2>
                
                <section className='flex flex-col gap-2'>
                <button disabled={loading} onClick={() => handleDelete(problem.id)} className='ds-btn bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-xl'>
                  Delete
                </button>
                <button onClick={() => handleSolve(problem.id)} className='ds-btn-primary py-2 px-4'>
                  Solve
                </button>
                </section>
              ...
              <p className="text-xs text-slate-400">Created at: {new Date(problem.createdAt).toLocaleDateString()}</p>
              {/* Preview images */}
              ...
              {/* Add more details or buttons as needed */}
              {
                problem.solution && (
                  <>
                  <p className="text-sm text-slate-700 mb-4 px-4">Solution: </p>
                  <p className="py-2 px-4 w-full text-sm text-slate-700 rounded-b-lg bg-slate-100">{problem.solution}</p>
                  </>
                )
              }
            </div>
      ...
      {!loading && problems.length > perPage && (
        <div className="mt-4 px-4 py-3 flex items-center justify-center border-t border-slate-200 sm:px-6">
